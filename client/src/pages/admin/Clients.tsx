            ...
            <tbody>
              {filtered.map(c => (
                <Tr key={c.id}>
                  ...
                  <Td><span className="font-mono text-xs" style={{ color: "var(--muted)" }}>{c.cnpj || "—"}</span></Td>
                  <Td><span className="text-sm">{c.email || "—"}</span></Td>
                  <Td><span className="text-sm">{c.phone || "—"}</span></Td>
                  <Td><span className="text-xs px-2 py-0.5 rounded-md font-medium" style={{ background: "var(--surface2)", border: "1px solid var(--border)" }}>{c.origin || "—"}</span></Td>
