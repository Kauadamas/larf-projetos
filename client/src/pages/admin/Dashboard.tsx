...
export default function Dashboard() {
  ...
  if (isLoading) return <div className="p-6 text-sm" style={{ color: "var(--muted)" }}>Carregando...</div>;
  ...
  return (
    <div className="p-6 max-w-7xl">
      <div className="mb-5">
        ...
        <p className="text-sm mt-0.5" style={{ color: "var(--muted)" }}>Visão geral da operação LARF</p>
      </div>
              ...
              <tbody>
                {stats.recentProjects.map(p => (
                  <Tr key={p.id} onClick={() => navigate("/admin/projects")}>
                    ...
                    <Td><span className="text-xs" style={{ color: "var(--muted)" }}>{clientMap[p.clientId ?? 0] || "—"}</span></Td>
