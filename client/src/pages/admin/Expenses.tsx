                ...
                <tbody>
                  {expenses.map(e => (
                    <Tr key={e.id}>
                      ...
                      <Td><span className="text-xs px-2 py-0.5 rounded-md font-medium" style={{ background: "var(--surface2)", border: "1px solid var(--border)" }}>{e.category}</span></Td>
                      <Td><span className="text-sm">{fmtDate(e.date)}</span></Td>
                      <Td><span className="text-xs" style={{ color: "var(--muted)" }}>{projectMap[e.projectId ?? 0] || "—"}</span></Td>
