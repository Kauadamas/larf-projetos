            ...
            <tbody>
              {invoices.map(i => (
                <Tr key={i.id}>
                  ...
                  <Td><span className="text-xs" style={{ color: "var(--text-lo)" }}>{clientMap[i.clientId ?? 0] || "—"}</span></Td>
                  <Td><span className="text-xs" style={{ color: "var(--text-lo)" }}>{projectMap[i.projectId ?? 0] || "—"}</span></Td>
