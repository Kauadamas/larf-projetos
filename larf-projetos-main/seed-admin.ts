 ...
 *
 * Com credenciais customizadas (Railway):
...
async function seed() {
  ...
  if ((rows as any[]).length > 0) {
    ...
    await conn.execute(
      // role: 'superadmin' | status: 'active' — correto para o schema atual
      "INSERT INTO users (name, email, passwordHash, role, status) VALUES (?, ?, ?, 'superadmin', 'active')",
